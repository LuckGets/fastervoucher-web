import { paths } from '@/config/path';

interface footerLinks {
  href: string;
  src: string;
  label: string;
}

export const footerLinks: footerLinks[] = [
  {
    href: paths.app.home.path,
    src: 'data:image/svg+xml;base64,',
    label: 'Home',
  },
  {
    href: paths.app.cart.path,
    src: 'data:image/svg+xml;base64,',
    label: 'Cart',
  },
  {
    href: paths.app.history.path,
    src: 'data:image/svg+xml;base64,',
    label: 'History',
  },
  {
    href: paths.app.user.path,
    src: 'data:image/svg+xml;base64,',
    label: 'User',
  },
];
