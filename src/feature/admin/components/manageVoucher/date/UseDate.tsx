...
const UseDate = () => {
  ...
  const [useDate, setUseDate] = useState<string>(voucher?.useDateStart || '');
  const [endDate, setEndDate] = useState<string>(voucher?.useDateEnd || '');
  ...
  const handleCancel = () => {
    setUseDate(voucher?.useDateStart || '');
    setEndDate(voucher?.useDateEnd || '');
    setIsEditing(false);
  };
